# PredSX — VPS Monitoring & Alerting Guide

> Covers how to set up, manage, and maintain the Discord alert system and cron job that monitors the PredSX VPS health.
...
2. Click **New Webhook**
3. Give it a name: `PredSX Monitor`
...
if [ -n "$ALERTS" ]; then
  MSG="PredSX Alert $(date '+%Y-%m-%d %H:%M:%S'): ${ALERTS}"
  curl -s -X POST "$WEBHOOK" \
    -H "Content-Type: application/json" \
    -d "{\"content\": \"${MSG}\"}"
fi
"""
with open('/usr/local/bin/predsx-health.sh', 'w') as f:
    f.write(script)
print("Done")
PYEOF
```
...
```bash
sudo chmod +x /usr/local/bin/predsx-health.sh
```

Then update the webhook URL inside the script:

```bash
sudo sed -i 's|YOUR_DISCORD_WEBHOOK_URL|https://discord.com/api/webhooks/YOUR_ID/YOUR_TOKEN|' /usr/local/bin/predsx-health.sh
```
...
curl -s -X POST "YOUR_DISCORD_WEBHOOK_URL" \
  -H "Content-Type: application/json" \
  -d '{"content": "PredSX test alert - Discord webhook is working!"}'
```
...
```bash
sudo sed -i 's/THRESHOLD_DISK=80/THRESHOLD_DISK=1/' /usr/local/bin/predsx-health.sh
sudo bash /usr/local/bin/predsx-health.sh
```

You should receive a Discord alert like:
```
PredSX Alert 2026-06-16 17:08:09: [DISK] Usage is 57%
```

Reset the threshold back to 80 after testing:

```bash
sudo sed -i 's/THRESHOLD_DISK=1/THRESHOLD_DISK=80/' /usr/local/bin/predsx-health.sh
```
...
```bash
(crontab -l 2>/dev/null; echo "0 * * * * /usr/local/bin/predsx-health.sh") | crontab -
```
...
Expected output:
```
0 * * * * /usr/local/bin/predsx-health.sh
```
...
```bash
sudo sed -i 's|https://discord.com/api/webhooks/OLD_ID/OLD_TOKEN|https://discord.com/api/webhooks/NEW_ID/NEW_TOKEN|' /usr/local/bin/predsx-health.sh
```
...
```bash
cat /usr/local/bin/predsx-health.sh
```
...
Delete the line with `predsx-health.sh`, save and exit.
