...
Returns `404 market not found` if the market ID is not in Redis (`pmaxis:markets` set).
...
Live orderbook snapshot, served verbatim from Redis (`pmaxis:orderbook:{id}`).
...
[
  ...
  {
    "type": "pmaxis.signal.volume_spike",
    "market_id": "0x9f8e7d6c5b4a3928170605040302010f9e8d7c6b",
    "value": 3.41,
    "severity": "medium",
...
```yaml
# .env
ALLOWED_ORIGINS=https://app.pmaxis.com,https://pmaxis.com
```
