# PMAxis — VPS Deployment Guide

> Full step-by-step guide for deploying PMAxis to a Hetzner (or any Ubuntu) VPS.
...
Open PowerShell in `C:\path\to\PMAxis\pmaxis\` and run:
...
3. Tags each image as both `:latest` and `:<git-hash>` (e.g. `pmaxis-api:cc27bce`)
...
```powershell
ssh -i $env:USERPROFILE\.ssh\your_ssh_key user@YOUR_VPS_IP "mkdir -p ~/pmaxis"
```
...
From `C:\path\to\PMAxis\pmaxis\`:

```powershell
scp -i $env:USERPROFILE\.ssh\your_ssh_key -r .\deployments\package\ user@YOUR_VPS_IP:~/pmaxis/
```
...
Each image prints `Loaded image: pmaxis-xxx:latest` when done. Takes 1–2 minutes.
...
```
pmaxis-api        latest    abc123   2 minutes ago   25MB
pmaxis-storage    latest    abc123   2 minutes ago   22MB
pmaxis-processor  latest    abc123   2 minutes ago   21MB
pmaxis-ingestion  latest    abc123   2 minutes ago   20MB
pmaxis-discovery  latest    abc123   2 minutes ago   19MB
```
...
> **Container naming:** Docker names containers using the compose project name as a prefix. When the compose files live in `~/pmaxis/package/`, Docker uses `package` as the project name — so all containers are `package-*`, not `pmaxis-*`.
...
{"time":"...","level":"INFO","msg":"metadata consumer started","group":"pmaxis-storage-dis"}
```
...
> **ClickHouse database name:** All tables live in the `default` database (not `pmaxis`). Always use `default.market_metadata`, `default.events_raw`, etc. in manual queries.
...
|----------|---------|----------|-------------|
| `ALLOWED_ORIGINS` | `https://app.pmaxis.com` | Yes | Comma-separated allowed CORS origins. Use `*` only for fully public APIs. |
...
docker build -t pmaxis-discovery:latest -f services/discovery/Dockerfile .
docker save pmaxis-discovery:latest -o deployments\package\discovery.tar
scp -i $env:USERPROFILE\.ssh\your_ssh_key .\deployments\package\discovery.tar user@YOUR_VPS_IP:~/pmaxis/package/
```

```bash
# VPS — load and recreate only that container
docker load -i ~/pmaxis/package/discovery.tar
cd ~/pmaxis/package
...
**Script location:** `/usr/local/bin/pmaxis-health.sh`
...
```bash
sudo sed -i 's|https://discord.com/api/webhooks/OLD|https://discord.com/api/webhooks/NEW|' /usr/local/bin/pmaxis-health.sh
```
