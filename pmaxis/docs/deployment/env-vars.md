# PMAxis — Environment Variables Reference
...
| `CLICKHOUSE_DB` | `default` | ClickHouse database name. All PMAxis tables live in `default` — do not change this. |
...
|----------|--------------|----------|----------|
| `MARKET_DISCOVERY_TOPIC` | `pmaxis.markets.discovered` | Discovery | Storage |
| `TOKEN_EXTRACTOR_TOPIC` | `pmaxis.tokens.extracted` | Discovery | Ingestion |
| `WS_RAW_TOPIC` | `pmaxis.ws.raw` | Ingestion | Processor, Storage |
| `TRADES_LIVE_TOPIC` | `pmaxis.trades.live` | Ingestion, Processor | Storage, API (WS) |
| `ORDERBOOK_UPDATES_TOPIC` | `pmaxis.orderbook.updates` | Processor | Storage, API (WS) |
| `PRICES_TOPIC` | `pmaxis.prices` | Processor | API (WS) |
| `SIGNALS_TOPIC` | `pmaxis.signals.live` | Processor | API (WS) |
| `ONCHAIN_TRADES_TOPIC` | `pmaxis.trades.onchain` | Ingestion | Processor, Storage |
| `TRADES_BACKFILL_TOPIC` | `pmaxis.trades.backfill` | Storage (backfill) | Storage (reader) |
| `METRICS_TRADES_TOPIC` | `pmaxis.metrics.trades` | Ingestion | — |
...
| `MARKET_DISCOVERY_TOPIC` | `pmaxis.markets.discovered` | No | Kafka topic for discovered market events |
| `TOKEN_EXTRACTOR_TOPIC` | `pmaxis.tokens.extracted` | No | Kafka topic for extracted YES/NO token pairs |
...
| `TOKEN_EXTRACTOR_TOPIC` | `pmaxis.tokens.extracted` | No | Consumed to learn which token IDs to subscribe to |
| `WS_RAW_TOPIC` | `pmaxis.ws.raw` | No | Topic for raw WebSocket payloads |
| `TRADES_LIVE_TOPIC` | `pmaxis.trades.live` | No | Topic for normalized trade events |
| `ONCHAIN_TRADES_TOPIC` | `pmaxis.trades.onchain` | No | Topic for on-chain Polygon trade events |
| `METRICS_TRADES_TOPIC` | `pmaxis.metrics.trades` | No | Topic for trade metrics aggregation |
...
| `CLICKHOUSE_PASSWORD` | `` | No | ClickHouse password |
| `CLICKHOUSE_DB` | `default` | No | ClickHouse database |
| `WS_RAW_TOPIC` | `pmaxis.ws.raw` | No | Raw WebSocket events to process |
| `TRADES_LIVE_TOPIC` | `pmaxis.trades.live` | No | Output topic for normalized trades |
| `ORDERBOOK_TOPIC` | `pmaxis.orderbook.updates` | No | Output topic for orderbook updates |
| `PRICES_TOPIC` | `pmaxis.prices` | No | Output topic for calculated prices |
| `SIGNALS_TOPIC` | `pmaxis.signals.live` | No | Output topic for trading signals |
| `ONCHAIN_TRADES_TOPIC` | `pmaxis.trades.onchain` | No | On-chain trade events to process |
...
| `CLICKHOUSE_PASSWORD` | `` | No | ClickHouse password |
| `CLICKHOUSE_DB` | `default` | No | ClickHouse database |
| `POSTGRES_URL` | `postgres://postgres:postgres@localhost:5432/pmaxis` | Yes | PostgreSQL connection string. Used for resumable backfill offsets. In Docker Compose, set to `postgres://postgres:postgres@postgres:5432/pmaxis`. |
| `REDIS_ADDR` | `localhost:6379` | Yes | Redis address |
| `WS_RAW_TOPIC` | `pmaxis.ws.raw` | No | Raw WebSocket events to persist |
| `TRADES_BACKFILL_TOPIC` | `pmaxis.trades.backfill` | No | Historical backfill topic |
| `TRADES_LIVE_TOPIC` | `pmaxis.trades.live` | No | Live trades to persist |
| `ORDERBOOK_UPDATES_TOPIC` | `pmaxis.orderbook.updates` | No | Orderbook snapshots to persist |
| `MARKET_DISCOVERY_TOPIC` | `pmaxis.markets.discovered` | No | Market metadata to persist |
| `ONCHAIN_TRADES_TOPIC` | `pmaxis.trades.onchain` | No | On-chain trades to persist |
...
| `TRADES_LIVE_TOPIC` | `pmaxis.trades.live` | No | Kafka topic the WebSocket broadcaster subscribes to for live trade events |
| `ORDERBOOK_UPDATES_TOPIC` | `pmaxis.orderbook.updates` | No | Kafka topic for live orderbook events |
| `PRICES_TOPIC` | `pmaxis.prices` | No | Kafka topic for live price events |
| `SIGNALS_TOPIC` | `pmaxis.signals.live` | No | Kafka topic for live signal events |
...
POSTGRES_URL=postgres://postgres:postgres@postgres:5432/pmaxis

# API Security (required for production)
ALLOWED_ORIGINS=https://app.pmaxis.com
...
> **Security note:** Never commit `.env` to git. The `.gitignore` in `pmaxis/` excludes `deployments/.env` and `deployments/package/.env`. Verify with `git status` before committing.
