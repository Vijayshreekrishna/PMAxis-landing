...
```
pmaxis-api:latest          ← always points to the most recent build
pmaxis-api:cc27bce         ← permanent tag for this specific commit
```
...
Output example:
```
pmaxis-api        latest    sha256:abc   10 minutes ago   25MB
pmaxis-api        cc27bce   sha256:abc   10 minutes ago   25MB
pmaxis-api        e71dc92   sha256:xyz   2 days ago       24MB
```
...
Verify the change:
```bash
grep "pmaxis-" docker-compose.yml
```

Expected output (all 5 services now pointing at the old hash):
```yaml
image: pmaxis-api:e71dc92
image: pmaxis-discovery:e71dc92
image: pmaxis-ingestion:e71dc92
image: pmaxis-processor:e71dc92
image: pmaxis-storage:e71dc92
```
...
sed -i 's/pmaxis-api:latest/pmaxis-api:OLDHASH/' docker-compose.yml
...
```bash
docker images | grep pmaxis | sort
```
