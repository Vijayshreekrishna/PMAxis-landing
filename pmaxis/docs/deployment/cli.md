# PMAxis — CLI Tool Reference

> The `cmd/pmaxis` CLI lets you query live market data from the terminal without a browser.
> It talks to the PMAxis API (`hub-api`) over HTTP and renders output as a formatted table or JSON.
...
```powershell
# From the PMAxis root directory
cd pmaxis
go run ./cmd/pmaxis --help
```

Or build a binary:
```powershell
go build -o pmaxis.exe ./cmd/pmaxis
```
...
|------|---------|-------------|
| `--api-url` | `http://localhost:8080` | Base URL of the PMAxis API. Override to point at your VPS. |
| `--json` | `false` | Output raw JSON instead of a formatted table. Useful for piping to `jq`. |

**Point at your VPS:**
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 markets
```
...
**JSON output:**
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 --json markets
```
```json
[
  {
...
```bash
pmaxis trades 558935
pmaxis --api-url http://YOUR_VPS_IP:8088 trades 558935
```
...
**JSON output:**
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 --json trades 558935
```
...
```bash
pmaxis orderbook 558935
pmaxis --api-url http://YOUR_VPS_IP:8088 orderbook 558935
```
...
```bash
pmaxis price 558935
pmaxis --api-url http://YOUR_VPS_IP:8088 price 558935
```
...
**JSON output:**
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 --json price 558935
```
```json
{
  "market_id": "558935",
...
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 --json markets | python3 -c \
...
```bash
ID=558935
pmaxis --api-url http://YOUR_VPS_IP:8088 price $ID
pmaxis --api-url http://YOUR_VPS_IP:8088 trades $ID
```

**Pipe JSON to `jq` for filtering:**
```bash
pmaxis --api-url http://YOUR_VPS_IP:8088 --json markets | jq '.data[] | select(.status == "ACTIVE") | .title'
```
