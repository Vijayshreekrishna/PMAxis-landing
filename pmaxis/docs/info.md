...
**PMAxis** is a production-grade, real-time prediction market data platform. It ingests live market data from Polymarket, normalizes and analyzes it for trading signals, and exposes it through a unified REST API and WebSocket gateway.

This repo is backend-only: a highly concurrent, event-driven system ("The Brain") built entirely in Go 1.23. The standalone Next.js frontend (PMAxis-Stat) has been retired and removed from this stack.
...
- **CLI Tool** (`cmd/pmaxis`): Developer tool for querying market data from the terminal.
...
The 5 Core Hubs live in `pmaxis/services/`:
...
Run all commands from the root `PMAxis` folder (`C:\path\to\PMAxis`).

### Start the Stack
```bash
docker compose -f pmaxis/deployments/docker-compose.yml up --build -d
```

### Stop the Stack
```bash
docker compose -f pmaxis/deployments/docker-compose.yml down
```
...
```bash
# All services
docker compose -f pmaxis/deployments/docker-compose.yml logs -f
...
```bash
docker build -t pmaxis-api -f pmaxis/services/api/Dockerfile .
```
...
```powershell
# Start everything (from pmaxis/ subfolder)
docker-compose -f deployments/docker-compose.yml up --build

# Build a single service image
docker build -t pmaxis-api -f services/api/Dockerfile .
```
...
| Kafka retention | **7-day TTL** (`168h`), **512 MB max per partition**, 100 MB segment size | Transit pipe only — not for replay. Topic-level override on `pmaxis.ws.raw` keeps raw WS data to 24h / 300 MB per partition |
...
Polymarket API
      │
      ▼
[market-discovery] ──► pmaxis.markets.discovered
      │
[token-extractor]  ──► pmaxis.tokens.extracted
      │
[stream]           ──► Raw WebSocket events
      │
      ├──► [trade-engine]      ──► pmaxis.trades
      ├──► [orderbook-engine]  ──► pmaxis.orderbook
      └──► [price-engine]      ──► pmaxis.prices
                │
...
**CLI Tool** (`cmd/pmaxis`): subcommands `markets`, `trades`, `orderbook`, `price`; rich table output via `tablewriter`; JSON pipe mode.
...
2. **Missing `cmd/pmaxis` module** — `go.work` requires all referenced modules present; added `COPY cmd/pmaxis cmd/pmaxis` to all Dockerfiles.
...
- Git initialized at `C:/path/to/PMAxis/`
...
| Issue | Fix |
|---|---|
| No `.dockerignore` — entire `pmaxis/` repo sent as Docker build context | Created `pmaxis/.dockerignore` excluding `docs/`, `deployments/`, `.git`, secrets |
...
**Why `.dockerignore` matters:** Without it, the entire `pmaxis/` directory — including `docs/`, `deployments/`, `.git/`, and all Markdown files — is sent to the Docker daemon as the build context before each image build. This slows down every `docker build` call (network transfer to daemon) and risks accidentally including secrets or environment files inside images. The `.dockerignore` reduces the context to only what Go actually needs: `libs/`, `services/`, `cmd/`, `go.work`, `go.work.sum`.
...
**Why git hash tagging matters:** Every `build-and-ship.bat` run previously overwrote `:latest` on all 5 images with no record of what changed. If a deploy broke something, there was no prior image to roll back to — you'd have to rebuild from source on VPS. Now each build also tags `:<short-hash>` (e.g. `pmaxis-api:fc0dd1c`). The `:latest` tag still advances as normal, but the hash tag is permanent. Rolling back is: stop the stack, change image tags in `docker-compose.yml` to the old hash, bring the stack back up.
...
- API `GetPrice` handler read from: `pmaxis:price:<marketID>`

Fix: Changed `rest.go` line 333 from `pmaxis:price:` to `live:price:`.
...
| ClickHouse database is `default`, not `pmaxis` | All tables live in `default`. Queries must use `default.market_metadata` etc. |
| Container prefix is `package-*`, not `pmaxis-*` | Docker uses the compose file's directory name (`package`) as the project prefix |
...
**Cause 1 — Kafka `pmaxis.ws.raw` topic grew to 7.5 GB (no per-topic limits)**
...
- `pmaxis/deployments/package/docker-compose.yml` — new `uptime-kuma` service block + `uptime_kuma_data` volume
- `pmaxis/docs/uptime-kuma.md` — new doc: full setup guide, all 12 monitor configs, Discord alert setup, status page creation, maintenance commands
...
| System | Covers |
|---|---|
| Cron script (`pmaxis-health.sh`) | OS-level: disk %, RAM %, swap %, exited containers |
...
| `cmd/pmaxis` — CLI tool | ✅ Implemented |
...
- [cli.md](cli.md) — CLI tool (`cmd/pmaxis`) subcommand reference
