# PMAxis — Uptime Kuma Status Dashboard

> Uptime Kuma is the visual monitoring and status-page system for PMAxis. It runs as a Docker container inside the same network as all other services, pings each one every 60 seconds, keeps history, and alerts on Discord when anything goes down.
...
| Public status page | `http://167.233.97.217:3001/status/pmaxis` |
| Internal port | `3001` |
...
| VPS compose path | `/home/vijay/pmaxis/package/docker-compose.yml` |

Uptime Kuma can reach all PMAxis services directly by their Docker service names (same internal network), so monitors point to internal hostnames like `hub-api:8088` rather than the public IP — faster, more reliable, no firewall concerns.
...
```bash
scp -i ~/.ssh/id_vsk pmaxis/deployments/package/docker-compose.yml vijay@167.233.97.217:/home/vijay/pmaxis/package/docker-compose.yml
```
...
4. Friendly Name → `PMAxis Discord`
5. Webhook URL → paste your Discord webhook URL (same one from `pmaxis-health.sh`)
...
3. Title → `PMAxis Status`
4. Content → `Real-time status of all PMAxis services`
5. Slug → `pmaxis`
6. Click **Create**
...
```
http://167.233.97.217:3001/status/pmaxis
```
...
All commands must be run from `/home/vijay/pmaxis/package` on the VPS.
...
PMAxis runs two complementary monitoring systems:
...
| **Cron script** (`pmaxis-health.sh`) | Checks OS-level metrics: disk %, RAM %, swap %, stopped containers — things Uptime Kuma cannot see | Discord |
