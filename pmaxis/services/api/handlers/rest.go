...
func (h *APIHandler) GetMarket(w http.ResponseWriter, r *http.Request) {
	...
	exists, err := h.Redis.SIsMember(ctx, "pmaxis:markets", id).Result()
	if err != nil {
...
func (h *APIHandler) GetOrderbook(w http.ResponseWriter, r *http.Request) {
	...
	data, err := h.Redis.Get(ctx, "pmaxis:orderbook:"+id).Result()
	if err != nil {
...
func (h *APIHandler) GetDebugOrderbook(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT data FROM events WHERE type='pmaxis.orderbook' AND market_id=? ORDER BY timestamp DESC LIMIT 1"
...
func (h *APIHandler) resolveCanonicalMarketID(ctx context.Context, marketID string) string {
	...
	if exists, err := h.Redis.SIsMember(ctx, "pmaxis:markets", marketID).Result(); err == nil && exists {
		return marketID
	}
...
func (h *APIHandler) GetMarketSummary(w http.ResponseWriter, r *http.Request) {
	...
	signals := make([]map[string]interface{}, 0)
	sigQuery := "SELECT type, value, severity, timestamp FROM events_raw WHERE type LIKE 'pmaxis.signal.%' AND market_id = ? ORDER BY timestamp DESC LIMIT 5"
