...
import (
	...
	"github.com/pmaxis/pmaxis/libs/crypto"
	...
	retry "github.com/pmaxis/pmaxis/libs/retry-utils"
...
func main() {
	...
	svc.Run(context.Background(), func(ctx context.Context) error {
		...
		pgConn := config.GetEnv("POSTGRES_URL", "postgres://postgres:postgres@localhost:5432/pmaxis")
...
func startNormalizerComponent(ctx context.Context, svc *service.BaseService, brokers string, ch clickhouse.Interface, rdb redisclient.Interface) {
	...
	wsTopic := config.GetEnv("WS_RAW_TOPIC", "pmaxis.ws.raw")
	backfillTopic := config.GetEnv("TRADES_BACKFILL_TOPIC", "pmaxis.trades.backfill")
	tradesLiveTopic := config.GetEnv("TRADES_LIVE_TOPIC", "pmaxis.trades.live")
	obTopic := config.GetEnv("ORDERBOOK_UPDATES_TOPIC", "pmaxis.orderbook.updates")
	discoveryTopic := config.GetEnv("MARKET_DISCOVERY_TOPIC", "pmaxis.markets.discovered")
	onChainTopic := config.GetEnv("ONCHAIN_TRADES_TOPIC", "pmaxis.trades.onchain")
...
func normalizeBackfill(evt schemas.HistoricalEvent) schemas.NormalizedEvent {
	...
	return schemas.NormalizedEvent{
		...
		EventType: "pmaxis.trades.backfill", Data: evt.Data, Timestamp: ts,
	}
}
...
func startBackfillComponent(ctx context.Context, svc *service.BaseService, brokers string) {
	topic := config.GetEnv("TRADES_BACKFILL_TOPIC", "pmaxis.trades.backfill")
