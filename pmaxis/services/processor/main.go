...
func main() {
	...
	svc.Run(context.Background(), func(ctx context.Context) error {
		...
		// --- TOPICS ---
		wsRawTopic := config.GetEnv("WS_RAW_TOPIC", "pmaxis.ws.raw")
		tradesLiveTopic := config.GetEnv("TRADES_LIVE_TOPIC", "pmaxis.trades.live")
		orderbookTopic := config.GetEnv("ORDERBOOK_TOPIC", "pmaxis.orderbook.updates")
			...
			if priceChanges, ok := payload["price_changes"].([]interface{}); ok && len(priceChanges) > 0 {
				...
				for _, item := range priceChanges {
					...
					if data, err := json.Marshal(update); err == nil {
						rdb.Set(ctx, "pmaxis:orderbook:"+marketID, data, 2*time.Hour)
					}
				}
				continue
			}
